...
double * RandomSearch::getBestSolution(bool & error)
{
	return bestSolution;
}

bool RandomSearch::run(int repeats) {
	bool changed = false;
	int solutions = 0;
	double *currentSolution = nullptr;
	double currentQuality = 0;
	...
	if(changed)
		std::cout <<"Nowe najlepsze rozwiazanie: "<< bestQuality << std::endl;


	return bestSolution;
}

double RandomSearch::getBestQuality(bool & error)
{
	return bestQuality;
}
