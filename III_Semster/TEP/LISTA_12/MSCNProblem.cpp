...
int MSCNProblem::checkValidityWithCorection(double *solution, bool &inputValid) {
	...
	//	std::cout << "nullptr" << std::endl;
		...
		search->setDeliverers(d);
		search->setFactories(f);
		search->setDistributors(m);
		search->setShops(s);
		search->seedProblem(d*f*m*s);
		solution = search->generateSolution(requiredSize);
		delete search;
		std::cout << "Corrected pointer" << std::endl;
		checkValidityWithCorection(solution, inputValid);
		return solutionNull;
	}
	...
	if (len != requiredSize) {
		...
		search->setShops(s);
		search->seedProblem(d*f*m*s);
		solution = search->generateSolution(requiredSize); //requeiredSize <<seed
		delete search;
		std::cout << "Corrected size" << std::endl;
		checkValidityWithCorection(solution, inputValid);
		return invalidLength;
	}
	...
	for (int i = 0; i < requiredSize; i++) {
		if (solution[i] < 0 || solution[i] > getMax(i) || solution[i] < getMin(i)) {
			if (solution[i] != 0) {
				inputValid = false;
				//               std::cout << "failed range" << std::endl;
				if (solution[i] < 0 && solution[i] != EOS) {
					solution[i] = 0;
				}
				else {
					if (solution[i] < getMin(i)) {
						solution[i] = getMin(i);
					}
					else {
						if (solution[i] > getMax(i)) {
							solution[i] = getMax(i);
						}
					}
				}
	...
	if (!inputValid) {
		std::cout << "Corrected range" << std::endl;
		checkValidityWithCorection(solution, inputValid);
		return failedRange;
	}
	...
	for (int y = 0; y < xf.getSizeY(); y++) {
		if ((xf - y) > (xd | y)) {
			...
			Random gen;
			int toCorrect = gen.getIntegerRandom(0, f);
			if (solution[toCorrect + y * m + f * d] > getMin(toCorrect + y * m + f * d)) {
				solution[toCorrect + y * m + f * d] -= (solution[toCorrect + y * m + f * d] - getMin(toCorrect + y * m + f * d))*correction;
			}
			toCorrect = gen.getIntegerRandom(0, d);
			if (solution[toCorrect + y * f] > getMin(toCorrect + y * f)) {
				solution[toCorrect + y * f] -= (solution[toCorrect + y * f] - getMin(toCorrect + y * f))*correction;
			}
			inputValid = false;
			//std::cout<<"failed1"<<std::endl;
		}
	...
	if (!inputValid) {
		std::cout << "Corrected Constraints_1" << std::endl;
		return failedConstraints;
	}
	...
	for (int y = 0; y < xm.getSizeY(); y++) {
		if ((xm - y) > (xf | y)) {
			Random gen;
			int toCorrect = gen.getIntegerRandom(0, m);
			if (solution[toCorrect + y * s + f * d + m * s] > getMin(toCorrect + y * s + f * d + m * s)) {
				solution[toCorrect + y * s + f * d + m * s] -= (solution[toCorrect + y * s + f * d + m * s] - getMin(toCorrect + y * s + f * d + m * s))*correction;
			}
			toCorrect = gen.getIntegerRandom(0, f);
			if (solution[toCorrect + y * f + f * d] > getMin(toCorrect + y * f + f * d)) {
				solution[toCorrect + y * f + f * d] -= (solution[toCorrect + y * f + f * d] - getMin(toCorrect + y * f + f * d))*correction;
			}
			inputValid = false;
		}
	}
	if (!inputValid) {
		std::cout << "Corrected Constraints_2" << std::endl;
		return failedConstraints;
	}
	...
	for (int y = 0; y < d; y++) {
		if ((xd - y) > sd->getElement(0, y)) {
			//          std::cout << "failed capacity" << std::endl;
			Random gen;
			int toCorrect = gen.getIntegerRandom(0, d);
			solution[toCorrect + y * f ] =0;
			inputValid = false;

		}
	}
	if (!inputValid) {
			std::cout << "Corrected Capacity_1" << std::endl;
			return failedCapacity;
	}
	...
	for (int y = 0; y < f; y++) {
		if ((xf - y) > sf->getElement(0, y)) {
			...
			Random gen;
			int toCorrect = gen.getIntegerRandom(0, f);
			solution[toCorrect + y * m + f * d] = 0;
			inputValid = false;
		}
	}
	if (!inputValid) {
			std::cout << "Corrected Capacity_2" << std::endl;
			return failedCapacity;
	}
	...
	for (int y = 0; y < m; y++) {
		if ((xm - y) > sm->getElement(0, y)) {
			...
			Random gen;
			int toCorrect = gen.getIntegerRandom(0, m);
			solution[toCorrect + y * s+ f * d + m * s] = 0;
			inputValid = false;
		}
	}
	if (!inputValid) {
			std::cout << "Corrected Capacity_3" << std::endl;
			return failedCapacity;
	}
	...
	for (int y = 0; y < s; y++) {
		if ((xm | y) > ss->getElement(0, y)) {
			//          std::cout << "failed capacity" << std::endl;
			Random gen;
			int toCorrect = gen.getIntegerRandom(0, s);
			solution[y + toCorrect* s + f * d + m * s] = 0;
			inputValid = false;
		}
	}
	if (!inputValid) {
			std::cout << "Corrected Capacity_4" << std::endl;
			return failedCapacity;
	}
